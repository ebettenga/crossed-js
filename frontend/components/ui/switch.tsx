...
const SwitchNative = React.forwardRef<SwitchPrimitives.RootRef, SwitchPrimitives.RootProps>(
  ({ className, ...props }, ref) => {
    ...
    const animatedRootStyle = useAnimatedStyle(() => {
      const inputColor = colorScheme === 'dark' ? 'rgb(39, 39, 42)' : 'rgb(228, 228, 231)';
      const primaryColor = 'rgb(139, 0, 0)'; // #8B0000 in RGB
      return {
      ...
      >
        ...
        >
          <Animated.View style={animatedThumbStyle}>
            <SwitchPrimitives.Thumb
              className={cn(
                'h-7 w-7 rounded-full shadow-md shadow-foreground/25 ring-0',
                'bg-white dark:bg-[#DDE1E5]'
              )}
            />
          </Animated.View>
        </SwitchPrimitives.Root>
