...
export class SocketEventService {
  ...
  private hasLocalSockets(roomName: string): boolean {
    const adapter =
      this.fastify.io.of("/").adapter.rooms as Map<string, Set<string>>;
    return adapter.get(roomName)?.size ? adapter.get(roomName)!.size > 0 : false;
  }
      ...
      switch (event.type) {
        case "user_event": {
          ...
          for (const userId of userIds) {
            ...
            if (isOnThisServer || this.hasLocalSockets(roomName)) {
              this.fastify.io.to(roomName).emit(eventName, data);
            }
          }
          break;
        }
