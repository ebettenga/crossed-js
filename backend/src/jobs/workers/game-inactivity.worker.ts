    ...
    async (job) => {
      ...
      if (room.status === "playing") {
        ...
        console.log("Room activity timestamps:", {
          roomId: room.id,
          lastActivityFromJob: new Date(job.data.lastActivityTimestamp).toISOString(),
        ...
        if (!room.last_activity_at) {
          console.log(`Room ${room.id} has no last_activity_at, setting to now`);
        ...
        const isInactive = job.data.lastActivityTimestamp === room.last_activity_at?.getTime() ||
          (job.data.lastActivityTimestamp && room.last_activity_at &&
           job.data.lastActivityTimestamp < room.last_activity_at.getTime());

        if (isInactive) {
          console.log(`No activity detected for room ${room.id} since last check`);
          ...
          if (letterToReveal) {
            ...
            console.log(`Room ${room.id} game finished status:`, isGameFinished);
            if (isGameFinished) {
            ...
            console.log(`Sending updated room state to all players for room ${room.id}`);
        ...
        if (room.status === "playing") {
          try {
            const now = Date.now();
            console.log(`Scheduling next inactivity check for room ${room.id}:`, {
              nextTimeout,
            ...
            console.error(`Failed to schedule next inactivity check for room ${room.id}:`, error);
            ...
            try {
              ...
              console.error(`Retry also failed for room ${room.id}:`, retryError);
              throw retryError;
            }
          }
        } else {
          console.log(`Not scheduling next check for room ${room.id} as status is ${room.status}`);
        }
      } else {
        console.log(`Skipping inactivity check for room ${room.id} as status is ${room.status}`);
      }
    },
    {
      connection: {
